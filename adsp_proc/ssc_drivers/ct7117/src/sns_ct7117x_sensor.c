  ...
  sns_std_sensor_config *decoded_payload)
{
  ...
  {
       SNS_UPRINTF(ERROR, this, "LSM decode error");
       return false;
  }
  return true;
}
  ...
  bool *sensor_tempture_client_present)
{
  ...
  *sensor_tempture_client_present = false;
                                                  ...
                                                  &state->my_suid, false))
  {
    ...
    SNS_UPRINTF(LOW, this, "temperature message id %d", request->message_id);
  ...
  SNS_UPRINTF(LOW, this, "temperature sample rete %d temperature present %d",
    ...
    ct7117x_sensor_type sensor_type)
{
  ...
  SNS_UPRINTF(LOW, this, "sensor type: %d %d", sensor_state->sensor, sensor_type);
 ...
 //   SNS_UPRINTF(HIGH, this,"debug inst_state->instance_is_ready_to_configure = false");
//  }
}
...
void ct7117x_sensor_process_suid_events(sns_sensor *const this)
{
      ...
      state->fw_stream->api->get_next_input(state->fw_stream))
  {
    ...
    if(SNS_SUID_MSGID_SNS_SUID_EVENT == event->message_id)
    {
      ...
      {
        SNS_UPRINTF(ERROR, this, "invalid datatype_name");
      }
    }
  }
}
    ...
    temp_power_rail_pending_state pwr_rail_pend_state)
{
  ...
  SNS_UPRINTF(LOW, this, "start power rail timer");
  ...
  if(NULL != state->timer_stream)
  {
    ...
    {
      SNS_UPRINTF(ERROR, this, "LSM timer req encode error");
    }
  }
  else
  {
    SNS_UPRINTF(ERROR, this, "create timer stream err");
  }
}
...
static sns_rc ct7117x_register_com_port(sns_sensor *const this)
{
    ...
    if (NULL == state->com_port_info.port_handle)
    {
        ...
        SNS_UPRINTF(LOW, this, "register com port success? rv=%d", rv);
        /* open the COM port*/
        if(rv == SNS_RC_SUCCESS)
        {
            rv = state->scp_service->api->sns_scp_open(
                state->com_port_info.port_handle);
            SNS_UPRINTF(LOW, this,
                       "open com port success> rv=%d, handle=%p", rv,
                       state->com_port_info.port_handle);
        }
        else
        {
            SNS_UPRINTF(ERROR, this,
                                                  ...
                                                  sns_sensor_event *event)
{
  ...
  SNS_UPRINTF(LOW, this, ">>ct7117x_sensor_process_registry_event = %d<<", event->message_id);              //514
  ...
  if(SNS_REGISTRY_MSGID_SNS_REGISTRY_READ_EVENT == event->message_id)
  {
    ...
    {
      SNS_UPRINTF(ERROR, this, "Error decoding registry event");
    }
    else
    {
      ...
      if(temp_cfg)
      {
        ...
        if(rv)
        {
          ...
          SNS_UPRINTF(LOW, this, "Registry read event for sensor:%d, received is_dri:%d ",
                                   state->sensor,
                                   state->is_dri);                                 
		  SNS_UPRINTF(LOW, this,"hardware_id:%lld ",state->hardware_id);
          SNS_UPRINTF(LOW, this, "resolution_idx:%d, supports_sync_stream:%d ",
      ...
      {
        SNS_UPRINTF(ERROR, this, "no find the group_name");
        rv = false;
      } 
    }
  }
  ...
  {
    SNS_UPRINTF(ERROR, this, "Received unsupported registry event msg id %u",
                             event->message_id);
  }
}
                              ...
                              sns_sensor_instance *instance)
{
  ...
  sns_request config;
  SNS_UPRINTF(LOW, this, "ct7117x_set_self_test_inst_config");
...
{
  SNS_UPRINTF(LOW, this, "publish available");
  sns_std_attr_value_data value = sns_std_attr_value_data_init_default;
  value.has_boolean = true;
  value.boolean = true;
  sns_publish_attribute(
...
static bool ct7117x_discover_hw(sns_sensor * const this)
{
    ...
    SNS_UPRINTF(LOW, this, "who am i is 0x%x",buffer[0]);

    if (rv == SNS_RC_SUCCESS && buffer[0] == TEMP_WHOAMI_VALUE)
    {
        SNS_UPRINTF(LOW, this, "RC_SUCCESS");
    }
    {
...
sns_rc ct7117x_sensor_notify_event(sns_sensor * const this)
{
  ...
  SNS_UPRINTF(LOW,this,"gu:enter sensor notify event");
  ...
  */
  ...
  if (NULL != state->timer_stream) 
  {
    ...
    while (NULL != event)
    {
      ...
      SNS_UPRINTF(LOW, this, "timer event message_id = %d", event->message_id);
      if(SNS_TIMER_MSGID_SNS_TIMER_SENSOR_EVENT == event->message_id)
      {
        if (pb_decode(&stream, sns_timer_sensor_event_fields, &timer_event))
        {
          ...
          if (state->power_rail_pend_state == POWER_RAIL_PENDING_INIT)
          {
            ...
            if (state->hw_is_present)
            {
               ...
               SNS_UPRINTF(ERROR, this, "sensor:%d initialize finished", state->sensor);
               ...
               if (NULL == instance)
               {
                   ...
                   SNS_UPRINTF(HIGH, this, "change-20260204-hyungchul: Auto-created instance to force start");
               }
            }
            else
            {
               rv = SNS_RC_INVALID_STATE;
               SNS_UPRINTF(ERROR, this, "temp HW absent");
            }
        ...
        {
          SNS_UPRINTF(ERROR, this, "pb_decode error");
        }
      }
  ...
  {
    SNS_UPRINTF(LOW, this, "registry data stream");
      ...
      NULL == state->pwr_rail_service)
  {
    ...
    state->pwr_rail_service->api->sns_register_power_rails(state->pwr_rail_service,
                                                           &state->rail_config);
    SNS_UPRINTF(LOW, this, "register power rails: rail vote:%d, num of rails:%d [1]%s [2]%s rv=%d",
    ...
    if( NULL != state->pwr_rail_service)
    {
      rv = state->pwr_rail_service->api->sns_vote_power_rail_update(
        ...
        SNS_UPRINTF(LOW, this, "vote power rails value:%d rv=%d",
    ...
    if (NULL == state->timer_stream)
    {
      SNS_UPRINTF(LOW, this, "here create the timer stream");
      stream_svc->api->create_sensor_stream(stream_svc,
                                            this,
                                            state->timer_suid,
                                            &state->timer_stream);
  ...
  {
    ct7117x_publish_available(this);
    SNS_UPRINTF(LOW, this, "sensor:%d initialize finished", state->sensor);
  }
  else
  {
    rv = SNS_RC_INVALID_STATE;
    SNS_UPRINTF(HIGH, this, "temp HW absent");
  }
#endif
  return rv;
}
                                                  ...
                                                  sns_physical_sensor_test_config *test_config)
{
  ...
  if(!pb_decode(&stream, sns_std_request_fields, decoded_request))
  {
    SNS_UPRINTF(ERROR, this, "decode error");
    return false;
  }
  return true;
}
...
static void ct7117x_cancel_power_rail_timer(sns_sensor *const this)
{
  ...
  SNS_UPRINTF(HIGH, this, "cancel_power_rail_timer: hw_id = %u, pend_state = %u",
...
static void ct7117x_check_instance(sns_sensor *const this)
{
                              ...
                              instance, &state->my_suid, true))
  {
    ...
    SNS_UPRINTF(HIGH, this, "change-20260204-hyungchul: Kept instance alive (ignored clientless removal)");
  }
}
                                                ...
                                                bool remove)
{
  ...
  SNS_UPRINTF(HIGH, this, "### set_client_request - temp_id=%d/%d remove=%u",
  ...
  else
  {
       ...
       && new_request->message_id != SNS_STD_MSGID_SNS_STD_FLUSH_REQ)
    {
      ...
      SNS_UPRINTF(HIGH, this,"debug set Power rails delta=%u o2i=%u",
    ...
    else if(NULL != instance)
    {
            ...
            new_request->message_id == SNS_STD_MSGID_SNS_STD_FLUSH_REQ)
      {
        if(NULL != exist_request)
        {
          ...
          instance = &sns_instance_no_error;
           SNS_UPRINTF(HIGH, this," debug sns_instance_no_error");
        }
        else
        {
          instance = NULL;
          SNS_UPRINTF(HIGH, this,"debug instance = NULL");
        }
        return instance;
      }
      else
      {
        reval_config = true;
        SNS_UPRINTF(HIGH, this," debug reval_config = true");
      }
    }
    ...
    if(NULL != instance)
    {
        ...
        (new_request->message_id == SNS_STD_SENSOR_MSGID_SNS_STD_ON_CHANGE_CONFIG)))
      {
        decode_request_rc = sns_ct7117x_handle_config(this, new_request);
         SNS_UPRINTF(HIGH, this," debug new_request->message_id == %d",new_request->message_id);
        if(SNS_RC_SUCCESS == decode_request_rc)
        {
          request_supported = true;
                 ...
                 (new_request->message_id == SNS_PHYSICAL_SENSOR_TEST_MSGID_SNS_PHYSICAL_SENSOR_TEST_CONFIG))
      {
        SNS_UPRINTF(LOW, this, "new_self_test_request received: Power rail [%u]",
        ...
        if(ct7117x_extract_self_test_info(this, instance, new_request))
          {
            ...
            SNS_UPRINTF(HIGH, this, "Self-test: Copying IRQ SUID. Valid? %d", 
      ...
      {
        SNS_UPRINTF(HIGH, this,"debug reval_config::%d && inst_state->instance_is_ready_to_configure::%d",
...
sns_rc ct7117x_temperature_init(sns_sensor * const this)
{
  ...
  SNS_UPRINTF(ERROR, this, "start register temp %d", state->hardware_id);
